 ...
 * change dir
 */
void ftserve_cd(int sock_control, int sock_data, char* arg)
{		
    int ret=chdir(arg);
    if(ret<0)
    {
...
/**
 * Send pwd of files in current directory
 * over data connection
 * Return -1 on error, 0 on success
 */
...
{	
	...
	if (strcmp(cmd, "QUIT")==0) {
		rc = 221;
	}else if( (strcmp(cmd, "PWDC")==0) || (strcmp(cmd, "PUTC")==0) ||
			(strcmp(cmd, "CDCC")==0) ||(strcmp(cmd, "LIST")==0) || (strcmp(cmd, "RETR")==0)) {
		rc = 200;
...
void ftserve_process(int sock_control)
{
	...
	if (ftserve_login(sock_control) == 1) {
		...
		send_response(sock_control, 421);	
		exit(0);
	}	
	...
	while (1) {
		...
		if (rc == 200 ) {
			...
			}else if (strcmp(cmd, "PUTC")==0) { // Do put <filename>
				ftserve_put(sock_control, sock_data, arg);
			}else if (strcmp(cmd, "CDCC")==0) { // Do cd  
				ftserve_cd(sock_control, sock_data, arg);
			}else if (strcmp(cmd, "PWDC")==0) { // Do pwd
